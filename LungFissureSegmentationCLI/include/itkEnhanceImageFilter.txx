/*=========================================================================
  ...
  Module:    $RCSfile: itkEnhanceImageFilter.txx,v $
  Language:  C++
...
=========================================================================*/
#ifndef _itkEnhanceImageFilter_txx
#define _itkEnhanceImageFilter_txx
#include "itkEnhanceImageFilter.h"
...
EnhanceImageFilter< TInputImage, TOutputImage >
::EnhanceImageFilter()
{
	...
	typename EigenValueImageType::Pointer strengthOutputImage = EigenValueImageType::New();
  	this->ProcessObject::SetNthOutput( 1,  strengthOutputImage.GetPointer() );
...
template< class TInputImage, class TOutputImage >
void EnhanceImageFilter< TInputImage, TOutputImage >
...
#endif
{
  ...
  if( m_Order >= 2 )
  {
	...
	for( outputIt.GoToBegin(),featuresIt.GoToBegin(),scalesIt.GoToBegin(),strengthIt.GoToBegin(); !featuresIt.IsAtEnd(); ++outputIt,++featuresIt,++scalesIt, ++strengthIt)
	{
		...
		//std::cout<<"normNoise: "<<normNoise<<std::endl;
		double tho0       = m_PSTh * trace0 * ( m_H * m_Sigma * m_Sigma );
		double tho1       = m_PSTh/normNoise;
		//std::cout<<"tho0: "<<tho0<<"tho1: "<<tho1<<std::endl;
...
::ComputeLSWeightsAndTrace( const InputImageSizeType& rcomp, unsigned int order)
{
	...
	for( unsigned int level = 0; level < this->m_NumberOfLevelSteps; level++ )  
	{
		...
		m_rvectorMap.insert( MatrixPairType(sigmaValue,rvector) );
...
const typename EnhanceImageFilter<TInputImage, TOutputImage >::FeaturesMapType *
EnhanceImageFilter< TInputImage, TOutputImage >::GetFeaturesOutput( void ) const
{
 ...
 */
template< typename TInputImage, typename TOutputImage >
const typename EnhanceImageFilter<TInputImage, TOutputImage >::ScalesImageType *
EnhanceImageFilter< TInputImage, TOutputImage >::GetScalesOutput( void ) const
{
 ...
 */
template< typename TInputImage, typename TOutputImage >
const typename EnhanceImageFilter<TInputImage, TOutputImage >::EigenValueImageType *
EnhanceImageFilter< TInputImage, TOutputImage >::GetFunctorOutput( void ) const
{
